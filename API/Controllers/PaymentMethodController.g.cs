...
{
    /// <summary>
    /// Controller responsible for managing paymentmethod-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class PaymentMethodController : ControllerBase
    {
        ...
        public PaymentMethodController(InventorymanagementsystemContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new paymentmethod to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.PaymentMethod.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<PaymentMethod>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.PaymentMethod.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.PaymentMethod.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] PaymentMethod updatedEntity)
        {
            ...
            this._context.PaymentMethod.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<PaymentMethod> updatedEntity)
        {
            ...
            var existingEntity = this._context.PaymentMethod.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.PaymentMethod.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
